...
const Services = () => {
  ...
  function handleOnChange(event) {
    ...
    switch (id) {
      case 'all':
        httpClient.get(`/service`).then((res) => {
          setServices(res.data);
        });
        break;
      default:
        httpClient.get(`categories/${id}/services`).then((res) => setServices(res.data));
        break;
    }
  }
  ...
  function renderServices(data) {
    return (
      <div className="serviceCard">
        {data.map((service, id) => {
          return <Service key={`${id}-service`} service={service} />;
        })}
      </div>
    );
  }
  ...
  return (
    <div className="services">
      <div className="Card content flex justifyBetween">
        ...
        {user ? (
          <Link to="/services/add" className="heroBtn">
            Post your service
          </Link>
        ) : (
          ''
