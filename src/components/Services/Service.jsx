...
const Service = ({ service, buttons }) => {
  ...
  return (
    <div
      key={`${key}-my-services`}
      className="Card expand contentCard"
    ...
    >
      <div className="serviceInfo flex flexColumn">
        ...
        <h5 className="serviceDetails" style={{ width: '280px' }}>
      ...
      <div className="userActions flex justifyBetween">
        {buttons ? (
          <>
            <Link
              className="heroBtn"
              onClick={(e) => userAction(e)}
              to={`/services/edit/${service.id}`}
            >
              Edit
            </Link>
            <button className="heroBtn btnRed" onClick={(e) => userAction(e, 'delete')}>
              Delete
            </button>
          </>
        ) : (
